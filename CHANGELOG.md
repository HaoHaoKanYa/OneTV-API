...
- 🎬 **VOD点播系统**：全新的点播源管理系统，独立于直播源系统运行
  - 32个精选优质点播源，包含拾光、欧歌、饭太硬、肥猫等特色源
  - 智能分类管理：综合影视、专业线路、魔改版本、多仓配置、特色专用
  - 自动质量验证和源可用性检测
  - TVBox标准格式配置文件生成
  - 私有Supabase存储桶，更安全可靠
  - 独立更新调度：每周日24点自动更新
...
- 🎬 **VOD系统架构**：完整的点播源管理技术栈
  - 独立的配置文件系统（vod_config.ini, vod_sources.txt）
  - 专用的GitHub Actions工作流（vod-update.yml）
  - 私有Supabase存储桶集成（vod-sources bucket）
  - TVBox格式JSON配置生成器
  - 源验证和质量检测系统
  - 7天更新周期的自动化管理
...
- 🎯 **目标用户**：中国大陆IPTV用户，专注高质量频道源和点播内容
- 📺 **频道重点**：央视频道、省级卫视、海外华语频道
- 🔄 **更新策略**：直播源每周五6点更新，点播源每周日24点更新
- 💾 **存储方案**：集成Supabase Storage，支持云端M3U和VOD配置文件管理
- 🚀 **部署方式**：支持GitHub Actions自动化部署
- 🎬 **点播服务**：提供32个优质点播源，支持TVBox等主流Android TV应用
...
- 更新频道别名数据，欢迎提供更多别名，参见：💖 [频道别名收集计划](https://github.com/HaoHaoKanYa/OneTV-API/discussions)
...
- ✨ 新增支持RTMP推流（工作流不支持），支持`Live/HLS`推流，订阅结果可转换为对应模式推流输出，也可通过`config`目录内创建`live`或
  `hls`目录定义读取本地视频源
- ✨ Docker镜像合并为`haohaokanya/onetv-api`，大小与精简版一致，不再区分完整版与精简版，`latest`为最新版，支持获取历史版本，如
  `1.6.2`
- ✨ 新增支持GUI最小化至系统托盘区运行
...
- ✨ Merged Docker images into `haohaokanya/onetv-api`, with the same size as the slim version. No longer distinguish between
...
- ✨ 新增支持`rtsp`协议接口
- ✨ 新增支持本地源频道名称模糊匹配
- ✨ 新增订阅源`HaoHaoKanYa/iptv-database`，来源于新仓库[IPTV-Database](https://github.com/HaoHaoKanYa/iptv-database)
- 🐛 修复支持含验证信息的接口匹配（#946）
...
- ✨ Added subscription source `HaoHaoKanYa/iptv-database`, from the new
  repository [IPTV-Database](https://github.com/HaoHaoKanYa/iptv-database)
...
- 🎉 预告：💻[OneTV-Web](https://github.com/HaoHaoKanYa/OneTV-Web)：IPTV电视直播源管理平台，支持在线播放等功能，开发中...
...
- 🎉 Preview: 💻[OneTV-Web](https://github.com/HaoHaoKanYa/OneTV-Web): IPTV live stream management platform, supports online
...
- ✨ 公众号详细教程文章已发布，欢迎关注`HaoHaoKanYa`公众号获取
- ✨ 新增支持`rtmp`协议接口（#780）
- ✨ 新增支持修改更新时间位置（`update_time_position`）（#755）
- ✨ 新增支持修改时区（`time_zone`）（#759）
- ✨ 更新组播源与酒店源离线数据，增加`广东移动组播RTP`（#773）
- ✨ 更新Github CDN代理地址（#796）
- ✨ GUI使用Github工作流基于源码自动构建并发布，唯一下载途径是[Release](https://github.com/HaoHaoKanYa/OneTV-API/releases)
  ，若安全软件有误报，请添加信任
- ✨ 增加版本信息打印输出
- ✨ 更新部分教程文档图片
...
- ✨ GUI is automatically built and released based on the source code using GitHub workflows, the only download method
  is [Release](https://github.com/HaoHaoKanYa/OneTV-API/releases). If there are false positives from security software, please
...
- ❤️ 推荐关注微信公众号（HaoHaoKanYa），订阅更新通知与使用技巧等文章推送，还可进行答疑和交流讨论
- ⚠️ 本次更新涉及配置变更，以最新 `config/config.ini` 为准，工作流用户需复制最新配置至`user_config.ini`
...
- ⚠️ Docker 新镜像仓库启用：haohaokanya/onetv-api（旧版的 tv-driver 改为：haohaokanya/onetv-api:latest，tv-requests 改为
  haohaokanya/onetv-api:lite），onetv-api:latest 为完整版、onetv-api:lite 为精简版，请使用新的名称命令进行拉取，旧仓库将不再维护
- ❤️ 新增微信公众号关注途径（公众号搜索：HaoHaoKanYa），推荐关注公众号，可订阅更新通知与使用技巧等文章推送，还可进行交流讨论
...
- ⚠️ The GitHub repository has been renamed to iptv-api. If you are using the old API address, please update it to the
  new one promptly.
- ⚠️ New Docker image repository is now active: haohaokanya/onetv-api (the old tv-driver is now haohaokanya/onetv-api:latest, and
  tv-requests is now haohaokanya/onetv-api:lite). onetv-api:latest is the full version, and onetv-api:lite is the lightweight
  version. Please use the new names to pull the images, as the old repository will no longer be maintained.
- ❤️ A new way to follow the WeChat official account (search for: HaoHaoKanYa) has been added. It is recommended to follow the
...
- ✨ 新增频道接口白名单：不参与测速，永远保留在结果最前面（#470）
  使用方法：
    ...
    2. 额外信息补充（如：广东珠江,http://xxx.m3u$!额外信息 ），更多接口白名单请至https:
       //github.com/HaoHaoKanYa/OneTV-API/issues/514 讨论
...
- ✨ Added channel interface whitelist: Not participating in speed testing, always kept at the very front of the
  ...
  Usage:
    ...
    2. Additional information can be appended (e.g., Guangdong Pearl River, http://xxx.m3u$! Additional Information) (
       #470). For more interface whitelists, please discuss at https://github.com/HaoHaoKanYa/OneTV-API/issues/514.
