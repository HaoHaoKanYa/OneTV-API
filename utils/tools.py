...
def get_epg_url():
    ...
    """
    if os.getenv("GITHUB_ACTIONS"):
        repository = os.getenv("GITHUB_REPOSITORY", "HaoHaoKanYa/OneTV-API")
            ...
            for line in file:
                ...
                if trimmed_line != "":
                    if "#genre#" in trimmed_line:
                        ...
                        m3u_output += f'#EXTINF:-1 tvg-name="{processed_channel_name}" tvg-logo="{join_url(config.cdn_url, logo_url)}"'
                        if current_group:
