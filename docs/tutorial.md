...
OneTV-API是一个可高度自定义的IPTV接口更新项目📺，自定义频道菜单，自动获取直播源，测速验效后生成可用的结果，可实现『✨秒播级体验🚀』
...
打开 https://github.com/HaoHaoKanYa/OneTV-API 点击`Star`收藏该项目（您的Star是我持续更新的动力）
...
1. 个人仓库命名，可按您喜欢的名字随意命名（最终直播源结果链接取决于该名称），这里以默认`OneTV-API`为例
2. 确认信息无误后，点击确认创建
...
![OneTV-API 更新软件](./images/ui.png 'OneTV-API 更新软件')
